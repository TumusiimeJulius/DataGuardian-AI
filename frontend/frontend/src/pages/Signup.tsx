...
import { FaShieldAlt, FaUser, FaEnvelope, FaLock, FaGoogle, FaEye, FaEyeSlash } from "react-icons/fa";
...
export default function Signup() {
      ...
      if (window.google) {
        ...
        if (clientId) {
          try {
            ...
            console.error("Error initializing Google GIS for signup", err);
          }
        }
      }
    };
  ...
  const handleGoogleCredentialResponse = async (response: any) => {
    ...
    try {
      ...
      if (data.token) {
        ...
        setSuccess("Account successfully registered & authenticated!");
        setTimeout(() => {
          navigate("/");
          window.location.reload();
        }, 800);
      } else {
        setError("Failed to register Google account credentials.");
      }
    } catch (err: any) {
  ...
  const handleSimulatedGoogleLogin = async () => {
    ...
    try {
      ...
      if (data.token) {
        localStorage.setItem("dg_auth_token", data.token);
        setSuccess("Simulated Google account registered & authenticated!");
        setTimeout(() => {
          navigate("/");
          window.location.reload();
        }, 800);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.token) {
        localStorage.setItem("dg_auth_token", data.token);
        setSuccess("Account created successfully! Welcome to DataGuardian AI.");
        setTimeout(() => {
          navigate("/");
          window.location.reload();
        }, 800);
      ...
      >
        ...
        >
          ...
          <p style={{ color: "#94a3b8", fontSize: "14px", margin: 0 }}>
            Join DataGuardian Data Observability Suite
          </p>
        </div>
